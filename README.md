...
У каждого коммита есть сообщение, которое кратко описывает внесённые изменения.
...
- использовать один стиль оформления;
- писать по существу.

Для сообщений на русском часто используют инфинитив:
...
```text
feat: добавить поиск пользователей
fix: исправить ошибку авторизации
docs: обновить README
refactor: упростить сервисный слой
...
В начале сообщения указывается идентификатор задачи.

```text
PROJ-123: добавить обработку ошибок
```

## GitHub Issues

Если коммит связан с issue, можно указать его номер:

```text
Исправить #334
```

GitHub автоматически свяжет коммит и задачу.
